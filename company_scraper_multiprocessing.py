...
def linkedin_login(webdriver, username, password):
    ...
    username_element.send_keys("jimmy.grodzki@hotmail.de")
    # password
    password_element = webdriver.find_element(By.XPATH, "//input[contains(@id, 'password')]")
    password_element.click()
    password_element.send_keys("!Smogmog92")
    # sign-in button
...
def linkedin_parsing_masterlist(dataframe):
    # PART TWO: Extract Company Information in detail
    email = ""
    password = ""
    driver = start_linkedin()
    linkedin_login(webdriver=driver, username=email, password=password)
    ...
    for index, row in tqdm(dataframe.iterrows()):
        ...
        try:
            ...
            employee_count = "Manual check needed"
        ...
        print(f"*** Summary --- Name: {company_name}, Employee Count: {employee_count}, Location: {location}, Employees on LinkedIn: {linkedin_employees}")
    driver.quit()
    excel_index = dataframe.first_valid_index()
    ...
    linkedin_company_detailed_df.to_csv(f"LinkedIn_Masterdata_Detailed_{excel_index}.csv")

if __name__ == "__main__":
    linkedin_company_df = pd.read_csv("LinkedIn_Masterdata.csv", index_col=0)
    df_list = [linkedin_company_df[0:int((len(linkedin_company_df))/4)],
               linkedin_company_df[int((len(linkedin_company_df))/4):int((len(linkedin_company_df))/4)*2],
               linkedin_company_df[int((len(linkedin_company_df))/4)*2:int((len(linkedin_company_df))/4)*3],
               linkedin_company_df[int((len(linkedin_company_df))/4)*3:int((len(linkedin_company_df))/4)*4]]
    pool = Pool(processes=4)
    pool.map(linkedin_parsing_masterlist, df_list)
